...
param(
    ...
    [string]$TenantName = 'M365x60502708',
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                Write-Log "Role assignment cmdlet executed successfully. Validating..." -Level Info
                ...
                if ($_.Exception.Message -like "*already exists*" -or $_.Exception.Message -like "*already assigned*") {
                    Write-Log "Role assignment already exists. Attempting validation..." -Level Warning
                    Wait-ForRoleAssignmentPropagation -InitialWaitSeconds 30 -MaxValidationAttempts 3
                }
                else {
                    # Automated approach failed - provide manual steps
                    Write-Log "Automated role assignment failed: $($_.Exception.Message)" -Level Error
                    Write-Log "======================================" -Level Info
                    ...
                    Write-Log "======================================" -Level Info
                    throw "Unable to configure Address Lists role. Manual intervention required."
                }
            }
        }
    }
    ...
    try {
        ...
        if ($existingPolicy) {
            ...
            New-AddressBookPolicy -Name $AddressBookPolicyName `
                -AddressLists "\Offline Global Address List", "\All Contacts", "\All Distribution Lists", "\All Rooms", "\All Users", "\All Groups", "\Public Folders" `
